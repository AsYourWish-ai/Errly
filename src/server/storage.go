...
// DeleteIssues removes issues (and their events) filtered by status.
// If status is empty, all issues are deleted.
// Returns the number of issues deleted.
func (s *Storage) DeleteIssues(status string) (int64, error) {
	tx, err := s.db.Begin()
	if err != nil {
		return 0, err
	}
	...
	// Delete events for matching issues first (FK constraint).
	_, err = tx.Exec(
		"DELETE FROM events WHERE issue_id IN (SELECT id FROM issues"+where+")",
		args...,
	)
	if err != nil {
		return 0, err
	...
	res, err := tx.Exec("DELETE FROM issues"+where, args...)
	if err != nil {
		return 0, err
	}
